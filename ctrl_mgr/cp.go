...
func LeafCPInitAllVer(ctx context.Context, leaf *model.Node, bfrtclient *bfrtC.Client, spines []_SpineConfig) {
	...
	*/
	...
	for _, server := range leaf.Children {
		...
		for _, uid := range model.UpstreamIDs {
			...
			ks = bfrtC.MakeKeys(k1, k2)
			d1 := bfrtC.MakeBytesData("port", uint64(model.LeafUpstreamPortMap[leaf.Index]))
...
func (cp *BfrtLeafCP_V1) Init() {
	...
	LeafCPInitAllVer(ctx, cp.leaf, cp.client, cp.spines)
}
...
func (cp *BfrtSpineCP_V1) Init() {
	...
	SpineCPInitAllVer(ctx, bfrtclient, spine)
...
func NewBfrtSpineCP_V2(client *bfrtC.Client, spine *model.Node) *BfrtSpineCP_V2 {
...
func NewBfrtLeafCP_V2(client *bfrtC.Client, leaf *model.Node, spines []_SpineConfig) *BfrtLeafCP_V2 {
...
func (cp *BfrtLeafCP_V2) Init() {
	...
	LeafCPInitAllVer(ctx, cp.leaf, bfrtclient, cp.spines)
